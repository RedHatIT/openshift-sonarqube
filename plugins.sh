...
do
	...
	if ! [[ -z "${MATCH_STRING}" ]]; then
		...
		if ! [[ -z "${DOWNLOAD_URL}" ]]; then
			printf "\t\t%-15s" ${PLUGIN}
			curl -L -sS -o $SONAR_HOME/extensions-init/plugins/${PLUGIN}.jar ${DOWNLOAD_URL} && printf "%10s" "DONE" || printf "%10s" "FAILED"
			printf "\n"
		else
		...
		if [[ "${PLUGIN}" == "buildbreaker" ]]; then
			...
			printf "\t\t%-15s" ${PLUGIN}
			curl -L -sS -o $SONAR_HOME/extensions-init/plugins/${PLUGIN}.jar ${BUILD_BREAKER_URL} && printf "%10s" "DONE" || printf "%10s" "FAILED"
			printf "\n"
		else
			printf "\t\t%-15s%10s\n" $PLUGIN "NOT FOUND"
		fi
